jQuery(document).ready(function($) {
	// datepicker field
	$('.celebExclusive-date').each(function(){
		var $this = $(this),
	...
	// timepicker field
	$('.celebExclusive-time').each(function(){
		var $this = $(this),
			format = $this.attr('rel');
	...
	// colorpicker field
	$('.celebExclusive-color-picker').each(function(){
		var $this = $(this),
			id = $this.attr('rel');

		$this.farbtastic('#' + id);
	});
	$('.celebExclusive-color-select').click(function(){
		$(this).siblings('.celebExclusive-color-picker').toggle();
		return false;
	});

	// add more file
	$('.celebExclusive-add-file').click(function(){
	...
	// delete file
	$('.celebExclusive-upload').delegate('.celebExclusive-delete-file', 'click' , function(){
		var $this = $(this),
			$parent = $this.parent(),
			data = $this.attr('rel');
		$.post(ajaxurl, {action: 'celebExclusive_delete_file', data: data}, function(response){
	...
	// reorder images
	$('.celebExclusive-images').each(function(){
		var $this = $(this),
			order, data;
		$this.sortable({
			...
			update: function (){
				...
				data = order + '|' + $this.siblings('.celebExclusive-images-data').val();

				$.post(ajaxurl, {action: 'celebExclusive_reorder_images', data: data}, function(response){
	...
	// thickbox upload
	$('.celebExclusive-upload-button').click(function(){
		...
		window.send_to_editor = function(html) {
			$('#celebExclusive-images-' + field_id).append($(html));
	...
	$('.ml-submit:first').append('<input type="hidden" name="field_id" value="' + field_id + '" /> <input type="submit" class="button" name="celebExclusive-insert" value="Insert selected images" />');
