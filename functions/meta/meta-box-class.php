...
class stalkerTheme_meta_box {
	...
	function js_css() {
		...
		wp_enqueue_style('stalkerTheme-meta-box', $base_url . '/meta-box.css');
		wp_enqueue_script('stalkerTheme-meta-box', $base_url . '/meta-box.js', array('jquery'), null, true);
	}
	...
	function insert_images() {
		if (!isset($_POST['stalkerTheme-insert']) || empty($_POST['attachments'])) return;
		...
		foreach ($_POST['attachments'] as $attachment_id => $attachment) {
			...
			$li .= "<a title='" . __('Delete this image') . "' class='stalkerTheme-delete-file' href='#' rel='$nonce|$post_id|$id|$attachment_id'>" . __('Delete') . "</a>";
	...
	function check_field_date() {
		if ($this->has_field('date') && $this->is_edit_page()) {
			...
			wp_enqueue_style('stalkerTheme-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('stalkerTheme-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
		}
	}
	...
	function check_field_time() {
		if ($this->has_field('time') && $this->is_edit_page()) {
			...
			wp_enqueue_style('stalkerTheme-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('stalkerTheme-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
			wp_enqueue_script('stalkerTheme-timepicker', 'https://github.com/trentrichardson/jQuery-Timepicker-Addon/raw/master/jquery-ui-timepicker-addon.js', array('rw-jquery-ui'));
		}
	}
// Check field image
	...
	function check_field_slider() {
		...
		// add style and script, use proper jQuery UI version
			wp_enqueue_script('stalkerTheme-jquery-ui-css', 'http://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/themes/base/jquery-ui.css');
			wp_enqueue_script('stalkerTheme-jquery-ui', 'https://ajax.googleapis.com/ajax/libs/jqueryui/' . $this->get_jqueryui_ver() . '/jquery-ui.min.js', array('jquery'));
	...
	function show() {
		...
		wp_nonce_field(basename(__FILE__), 'stalkerTheme_meta_box_nonce');
	...
	function show_field_begin($field, $meta) {
		echo "<th class='stalkerTheme-label'><label for='{$field['id']}'>{$field['name']}</label></th><td class='rw-field'>";
	}
	...
	function show_field_text($field, $meta) {
		...
		echo "<input type='text' class='stalkerTheme-text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_textarea($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<textarea class='stalkerTheme-textarea large-text' name='{$field['id']}' id='{$field['id']}' cols='60' rows='10'>$meta</textarea>";
		$this->show_field_end($field, $meta);
	}
	...
	function show_field_select($field, $meta) {
		...
		$this->show_field_begin($field, $meta);
		echo "<select class='stalkerTheme-select' name='{$field['id']}" . ($field['multiple'] ? "[]' id='{$field['id']}' multiple='multiple'" : "'") . ">";
...
function show_field_plaintext($field, $meta) {
	...
	function show_field_file($field, $meta) {
		...
		if (!empty($meta)) {
			...
			echo '<ol class="stalkerTheme-upload">';
			foreach ($meta as $att) {
				// if (wp_attachment_is_image($att)) continue; // what's image uploader for?
				echo "<li>" . wp_get_attachment_link($att, '' , false, false, ' ') . " (<a class='stalkerTheme-delete-file' href='#' rel='$nonce|{$post->ID}|{$field['id']}|$att'>" . __('Delete') . "</a>)</li>";
			}
			echo '</ol>';
		}
	...
	function show_field_image($field, $meta) {
		...
		echo "<input type='hidden' class='stalkerTheme-images-data' value='{$post->ID}|{$field['id']}|$nonce_sort' />
			  <ul class='stalkerTheme-images rw-upload' id='stalkerTheme-images-{$field['id']}'>";
		...
		// re-arrange images with 'menu_order', thanks Onur
			...
			foreach ($images as $image) {
				...
				echo "<li id='item_$image'>
						<img src='$src' />
						<a title='" . __('Delete this image') . "' class='stalkerTheme-delete-file' href='#' rel='$nonce_delete|{$post->ID}|{$field['id']}|$image'>" . __('Delete') . "</a>
		...
		echo "<a href='#' class='stalkerTheme-upload-button button' rel='{$post->ID}|{$field['id']}'>" . __('Add more images') . "</a>";
	}

	function show_field_color($field, $meta) {
		if (empty($meta)) $meta = '#';
		$this->show_field_begin($field, $meta);
		echo "<input class='stalkerTheme-color' type='text' name='{$field['id']}' id='{$field['id']}' value='$meta' size='8' />
			  <a href='#' class='stalkerThemel-color-select' rel='{$field['id']}'>" . __('Select a color') . "</a>
			  <a href='#' class='stalkerTheme-color-select' style='display: none;'>" . __('Hide Picker') . "</a>
	...
	function show_field_checkbox_list($field, $meta) {
		...
		$html = array();
		foreach ($field['options'] as $key => $value) {
			$html[] = "<input type='checkbox' class='stalkerTheme-checkbox_list' name='{$field['id']}[]' value='$key'" . checked(in_array($key, $meta), true, false) . " /> $value";
		}
	...
	function show_field_date($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='stalkerTheme-date' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}

	function show_field_time($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='stalkerTheme-time' name='{$field['id']}' id='{$field['id']}' rel='{$field['format']}' value='$meta' size='30' />";
		$this->show_field_end($field, $meta);
	}
function show_field_single_image($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<input type='text' class='stalkerTheme-single-image' name='{$field['id']}' id='{$field['id']}' value='$meta' size='18' />";
		echo "<input type='button' class='stalkerTheme-upload-single-image button-secondary' value='Choose Image'/><br />";
		$this->show_field_end($field, $meta);
	}
	
	function show_field_slider($field, $meta) {
		$this->show_field_begin($field, $meta);
		echo "<div class='stalkerTheme-slider' rel='{$field['id']}'></div>";		
	...
	function save($post_id) {
		...
		|| (!check_admin_referer(basename(__FILE__), 'stalkerTheme_meta_box_nonce'))		// verify nonce
		...
		foreach ($this->_fields as $field) {
			...
			// validate meta value
			if (class_exists('stalkerTheme_meta_box_Validate') && method_exists('stalkerTheme_meta_box_Validate', $field['validate_func'])) {
				$new = call_user_func(array('stalkerTheme_meta_box_Validate', $field['validate_func']), $new);
			}
